<template>
    <el-col>
        <el-card class="search-toolbar">
            ...
            <el-form :inline="true" ref="form" :model="listParams" :disabled="listLoading">
        ...
        <el-card>
            ...
            <el-table style="width: 100%" :data="list" v-loading="listLoading">
                ...
                <el-table-column label="状态" width="180">
                    <template #default="scope">
                        <el-switch v-model="scope.row.status" class="ml-2" active-color="#13ce66"
                            inactive-color="#ff4949" :active-value="1" :inactive-value="0"
                            :loading="scope.row.statusLoading" @change="handleStatusChange(scope.row)" />
                    </template>
                </el-table-column>
            ...
            <AppPagination v-model:page="listParams.page" v-model:limit="listParams.limit" :list-count="listCount"
                :load-list="loadList" :disabled="listLoading" />
...
const loadList = async () => {
    ...
    const data = await getAdmins(listParams).finally(() => { listLoading.value = false })
    data.data.list.forEach(item => { item.statusLoading = false })
...
const handleStatusChange = async (item: Admin) => {
    ...
    await updateAdminStatus(item.id, item.status).finally(() => { item.statusLoading = false })
